<template>
  <q-card bordered class="mycard cursor-pointer" @click="onClick">
    <q-card-section horizontal>
      <div :style="`background-color: ${hexColor}; width: 0.8em; min-width: 0.8em;`"></div>
      <q-card class="full-width q-pa-sm">
        <q-card-section horizontal class="q-gutter-xl">
          <q-icon
            :class="`${ctlCheck ? 'cursor-pointer' : 'cursor-pointer'} shadow-box text-primary`"
            :title="this.ctlCheck ? 'Feito' : 'Marcar como feito'"
            :name="this.ctlCheck ? 'check_circle' : 'radio_button_unchecked'"
            @click="onClickCheck"
          />
          <div>
            <div class="text-h6"><span class="text-bold">[{{ id }}]</span> {{ title }}</div>
        ...
        <span v-if="sizeDays" class="absolute-bottom-left" style="margin-left: 90px;">{{ `Tamanho ${qtdDias} ${ (qtdDias === '1' ? 'dia' : 'dias')}` }}</span>
      </q-card>
    </q-card-section>
  </q-card>
</template>
